...
def format_records(raw_data):
    ...
    for entry in raw_data_dict:
        raw_list.append(raw_data_dict[entry])
        if entry[0] not in key_data: key_data.append(entry[0])
    return raw_list,len(key_data)
...
def return_entry():
    input_list = [("CO.CatalogNumber LIKE'%s'",catalognum.get()),("CO.AltCatalogNumber LIKE'%s'", dao.get()), ("A.LastName LIKE '%s%s%s'", ('%', lastname.get(), '%')),
                  ("T.FullName LIKE'%s'",geography.get()),("YEAR(CE.StartDate) LIKE '%s'", year.get()), ("G.FullName LIKE '%s%s%s'", ('%', province.get(), '%'))]
    statement_list = []
    for field in input_list:
        if field[1] != "": statement_list.append(field[0] % field[1] + "AND ")
    return fetch_info(("".join(statement_list))[:-4])

# saves the query results to a .xls file named by the user
def save_to_file(records):
    ...
    if name != 'None':
        ...
        for i,row2 in enumerate(records):
            for j,col2 in enumerate(row2): ws.write(i+1,j,col2[0])
        wb.save("%s.xls" % name)
...
# displays the results of each query to a new tab inside the initial window
def display_results(tab_frame):
    ...
    if len(records[0]) != 0:
        ...
        for i, row in enumerate(records[0]):
            for j, data in enumerate(row): Label(frame, text="%s" % data[0], relief=RIDGE).grid(column=j,row=i+2,sticky=NSEW)
    else:
