...
#record[0] = ParentID, record[1] = FullName, record[2] = GeographyID
...
for country in recordsByRank[200]:  # searches each country 'subtree' for matching names
    for name1, name2 in itertools.combinations(([node.name for node in PreOrderIter((treeDict[country[2]][2]))]), 2):
        if name1[0] == name2[0]:
            print('Duplicate Found:', name1[0], name1[1], name2[1])
