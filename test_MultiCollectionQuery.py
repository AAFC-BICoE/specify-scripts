# Test for the MultiCollectionQuery script. Creates database with the tables needed to be used for the query
import MultiCollectionQuery, os, sqlite3, unittest

class TestDatabase(unittest.TestCase):

    def setUp(self):
        # Creates the test database and test tables, inserts sample data into test tables that will later be used for
        # The joining statement
        conn = sqlite3.connect("specifytest.db")
        cursor = conn.cursor()

        cursor.execute("CREATE TABLE collectionobject "
                       "(CatalogNumber, CollectionID, CollectingEventID, CollectionObjectID, AltCatalogNumber)")
        test_collection_object = [("1","4","10","16","test1"),("2","5","11","17","test2"),("3","6","12","18","test3")]
        cursor.executemany("INSERT INTO collectionobject VALUES (?,?,?,?,?)", test_collection_object)
        conn.commit()
        ...
        cursor.execute("CREATE TABLE collectingevent (CollectingEventID, StartDate DATE, LocalityID)")
        test_collecting_event = [("10", "2011-01-03","25"), ("11", "2001-09-08","26"), ("12", "2003-03-12","27")]
    ...
    def test_format_records_multiple_collectors(self):
        # Tests to confirm the function returns a properly formatted list when multiple collectors are passed in
        ...
        expected = ([["catnum", "dao", "collection", "test1, test2", "taxon", "locality", "year"]], 1)
        self.assertEqual(expected,actual)

    def test_format_records_multiple_catalog_nums(self):
        # Tests to confirm the function returns the correct number of unique catalognumbers
    ...
    def test_fetch_info_restriction_single_positive(self):
        # Tests to confirm properly formatted list is passed back containing the information from a restriction list
        # Containing a single restriction that is valid
        conn = sqlite3.connect("specifytest.db")
    ...
    def test_fetch_info_restriction_single_negative(self):
        # Tests to confirm empty list is passed back containing the information from a restriction list containing a
        # Single restriction that is invalid
        conn = sqlite3.connect("specifytest.db")
    ...
    def test_fetch_info_restriction_multi_positive(self):
        # Tests to confirm properly formatted list is passed back containing the information from a restriction list
        # Containing a multiple restrictions that are valid
        conn = sqlite3.connect("specifytest.db")
    ...
    def test_fetch_info_restriction_multi_negative(self):
        # Tests to confirm empty list is passed back containing the information from a restriction list containing a
        # Multiple restrictions that are invalid
        conn = sqlite3.connect("specifytest.db")
    ...
    def test_format_args_single(self):
        # Tests to confirm that a properly formatted list is returned when a single restriction is passed in
        test_catalognum = "test"
        test_dao, test_lastname, test_taxon, test_year, test_province = "","","","",""
        actual = MultiCollectionQuery.format_args(test_catalognum,test_dao,test_lastname,test_taxon,test_year,test_province)
        expected = "CO.CatalogNumber LIKE 'test' AND A.LastName LIKE '%%' AND G.FullName LIKE '%%' "
        self.assertEqual(expected,actual)

    def test_format_args_all(self):
        # Tests to confirm that a properly formatted list is returned when all possible restrictions are passed in
        ...
        expected = "CO.CatalogNumber LIKE 'test1' AND CO.AltCatalogNumber LIKE 'test4' AND A.LastName LIKE '%test3%' " \
                   "AND T.FullName LIKE 'test2' AND YEAR(CE.StartDate) LIKE 'test5' AND G.FullName LIKE '%test6%' "
        self.assertEqual(expected,actual)
