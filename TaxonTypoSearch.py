...
Creates a csv report of possible typos in taxon full names that are part of the same family and have the same author
first letter. Creates a taxon tree of parentID/taxonID relationships using the library 'anytree', then iterates through
each family subtree and takes the levenshtein distance between two taxon names that have the same author first letter.
...
# searches and compares within each family 'subtree' for matching first letter in Author then computes the LD
def search_tree(rank_records_dict,tree):
    typos = []
    family_names = rank_records_dict[140]
    for family in family_names:
        for name1, name2 in itertools.combinations(([node.name for node in PreOrderIter(tree[family[2]][2])]), 2):
            if (name1[2] is not None and name1[2] != "") and (name2[2] is not None and name2[2] != ""):
                ...
                if 0 < ld <= 2:
                    typos.append((family[1], name1[0], name2[0], name1[2], name2[2], name1[1],name2[1], ld))
...
# writes the typo data passed in to a csv file named 'TaxonTypoReport.csv'
def write_report(data):
    with open("TaxonTypoReport.csv", "w") as file_writer:
        writer = csv.writer(file_writer)
        writer.writerow(["Family Name", "Taxon Name 1", "Taxon Name 2","Author 1","Author 2", "TaxonID 1",
                         "TaxonID 2","Levenshtein Distance"])
        for row in data:
            writer.writerow(row)
    print("Report saved as 'TaxonTypoReport.csv'")

# calls on functions
def main():
    db = MySQLdb.connect("localhost", '''"MySQLusername", "MySQLpassword", "MySQLdatabaseName"''')
    print("Searching for possible typos in taxon full names within the same family")
