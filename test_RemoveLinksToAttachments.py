...
"""
import RemoveLinksToAttachments, os, test_db, sqlite3, unittest
...
class TestDatabase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cursor.execute("""CREATE TABLE attachment (AttachmentID, MimeType, data2, data3)""")
        conn.commit()
        test_attachment_data = [("1", "image/jpeg", "b", "c"), ("2", "otherfile", "e", "f"), ("2", "image/jpeg", "h", "i"), ("3", "image/jpeg", "k", "l")]
        cursor.executemany("""INSERT INTO attachment VALUES (?,?,?,?)""", test_attachment_data)
        conn.commit()

        cursor.execute("""CREATE TABLE collectionobjectattachment (AttachmentID,data1,data2,data3)""")
        conn.commit()
        test_collectionobjectattachment_data = [("1","a","b","c"),("2","b","c","d"),("4","c","d","e"),("3","d","e","f")]
        cursor.executemany("""INSERT INTO collectionobjectattachment VALUES (?,?,?,?)""" , test_collectionobjectattachment_data)
        conn.commit()

        cursor.execute("""CREATE TABLE testdata1 (AttachmentID,data1,data2,data3)""")
        test_testdata1 = [("1","a","b","c"),("2","b","c","d"),("4","c","d","e"),("3","d","e","f")]
        cursor.executemany("""INSERT INTO testdata1 VALUES (?,?,?,?)""", test_testdata1)
        conn.commit()

        cursor.execute("""CREATE TABLE testdata2 (AttachmentID,data1,data2,data3)""")
        test_testdata2 = [("1", "i", "b", "c"), ("2", "o", "e", "f"), ("2", "g", "h", "i"), ("3", "j", "k", "l")]
    ...
    def test_delete_attachments_attachment(self):
        ...
        RemoveLinksToAttachments.delete_attachments (conn, test_referenced_tables,test_attachments)
    ...
    def test_delete_attachments_no_conflicts(self):
        ...
        test_referenced_tables = [("testdata1",),("testdata2",),("collectionobjectattachment",)]
        test_attachments = [("1",),("2",),("3",)]
        actual = RemoveLinksToAttachments.delete_attachments (conn, test_referenced_tables,test_attachments)
        expected = []
        self.assertEqual(expected,actual)
    ...
    def test_delete_attachments_conflicts(self):
        ...
        conn = sqlite3.connect("specifytest.db")
        test_referenced_tables = [("testdata1",),("testdata",),("collectionobjectattachment",)]
        test_attachments = [("1",),("2",),("45",)]
        actual = RemoveLinksToAttachments.delete_attachments (conn, test_referenced_tables,test_attachments)
        expected = ["1","2","45"]
        self.assertEqual(expected,actual)
